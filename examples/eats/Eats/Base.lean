...
/-! The eats base as a value. Query names and arities are `query%`-derived
from the defs in `Eats/Queries.lean` and `Eats/OfferQueries.lean`;
closed-world arguments (city, weekday, diet, family) parse by variant
name, and the newtypes below get their own `CliArg` so
`openFor tiramisu fri 21:30` reads as written. The seed loads the
restaurants and the configurable offers. -/
