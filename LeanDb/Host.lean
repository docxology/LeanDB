...
def run (args : List String) : IO UInt32 := do
  let rec parse : List String → Except String (Option Nat × String × List String)
    | [] => .ok (none, "127.0.0.1", [])
    | "--port" :: p :: rest => do
        let (_, h, specs) ← parse rest
        match p.toNat? with
        | some n => return (some n, h, specs)
        | none => throw s!"--port expects a number, got {p}"
    | "--bind" :: h :: rest => do let (p, _, specs) ← parse rest; return (p, h, specs)
    | spec :: rest => do let (p, h, specs) ← parse rest; return (p, h, spec :: specs)
  let usage := "host --port <port> [--bind <host>] <name>=<exe>[,<arg>,…]…"
  ...
  | .ok (port?, host, specs) =>
      ...
      Http.serveHosted host port.toUInt16 list bases <| Json.mkObj [("ok", Json.bool true),
