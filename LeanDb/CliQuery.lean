...
into a `(name, List String → DbM Json)` registration: each argument is
parsed positionally through its type's `CliArg` instance (types drive
parsing — plan.md §4.2), the result renders through `QueryOut`. A base's
query list becomes `queries := [query% openTickets, query% slaBreached]` —
no hand-written flag parsing, and adding an argument to a query def
changes its CLI arity with no other edit.
-/
...
elab "query% " id:ident : term => do
  ...
  let (binderNames, binderTys) ← forallTelescope info.type fun xs body => do
  ...
  let stx ← `(Prod.mk ($nameLit : String)
