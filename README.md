...
let client ← Client.connect "path/to/tickets" (fingerprint Tickets.base.specs) ["--db", "data/tickets.sqlite"]
...
instances for its own newtypes (tickets' `Timestamp`).
...
| `examples/dashboard` | not a base | imports `tickets` and `eats`: in-process queries and a typed remote client |
